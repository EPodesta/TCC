...
arquiteturas \textit{multicore} e \textit{manycore}. Além disso, serão destacados alguns trabalhos de pesquisa sobre
o \mppa.
...
por uma \noc no formato de malha. O \fw \emph{FastFlow} provê padrões de \textit{design}
...
diferentes \textit{back-ends}, incluindo processadores \textit{multicore} e \gpus.
...
processadores \emph{manycore} em \hpc, além de discutir os desafios do
...
GHz com 8 núcleos, além de uma redução no consumo de energia em até 13 vezes em
...
três diferentes classes de aplicações (CPU-\textit{bound}\footnote{O desempenho
    da aplicação é limitado pela velocidade da CPU.},
\textit{memory-bound}\footnote{O desempenho da aplicação é limitado pela quantidade de memória disponível e velocidade de acesso à memória.}
e híbrida\footnote{Aplicação com características CPU-\textit{bound} e \textit{memory-bound}}.) usando plataformas paralelas, como o \mppa, e um multiprocessador \numa de
...
de trabalho \textit{memory-bound}, o sistema \numa obteve um melhor desempenho
...
Similar ao \fw \emph{SkePU}, o \pskel fornece suporte à aplicações
paralelas baseadas no padrão \stencil em ambientes heterogêneos CPU-GPU.
Contudo, o \pskel não oferece suporte à processadores
\textit{manycore}. Além disso, com base nos estudos sobre processadores
\textit{manycore}, pode-se perceber que o processador \mppa possui um consumo
energético inferior em relação aos processadores \textit{multicores}. O \mppa
apresenta limitações de memória que podem ser solucionadas com técnicas de
\textit{tiling}, como mostrado por~\emph{Lutz}~\etal~\cite{lutz13}. Mesmo
\emph{Thorarensen}~\etal~\cite{thoraransen16} apresentando uma proposta similar,
a arquitetura do \mppa é diferente do processador \emph{Myriad2}, apresentando
características e dificuldades de desenvolvimento diferentes.
Desta forma, a adaptação do \fw \pskel para um
processador de baixo consumo de energia e alto desempenho, como o \mppa, é
interessante para caracterizar um novo estudo sobre a utilização de \fws em
ambientes heterogêneos utilizando técnicas de \textit{tiling}.
