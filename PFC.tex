...
\textAbstract { Parallel applications can be classified according to computation
    ...
    reduce, pipeline, scan and stencil patterns. The latter is widely used in
    ...
    images, among others. The \pskel is a parallel programming framework
    ...
    computing \emph{kernel}, while the framework executes the parallel computing
    ...
    motivation for such work is related to the difficulty of stencil pattern
    based applications development on \mppa, caused by intrinsic architectural
    features that make the development of applications costly and error prone.
    ...
    such as the need for explicit communication between chip memories and
    computation distribution between processing cores.
    Several experiments were carried out with the proposed solution for the
    \mppa, comparing the existing multicore solution. The results showed that
    the proposed solution allows to reduce the stencil application energy
    consumption up to 1.45x, despite a performance loss up to 3.3x.}

\keywords {\textit{manycores}, \mppa, \textit{stencil}, PSkel.}
...
\lstinputlisting[language=C++, label=l1,caption=PSkelArray.h]{./anexos/PSkelArray.h}
\lstinputlisting[language=C++, label=l2,caption=PSkelArray.hpp]{./anexos/PSkelArray.hpp}
\lstinputlisting[language=C++, label=l3,caption=PSkelStencil.h]{./anexos/PSkelStencil.h}
\lstinputlisting[language=C++, label=l4,caption=PSkelStencil.hpp]{./anexos/PSkelStencil.hpp}
\lstinputlisting[language=C++, label=l5,caption=interface\_mppa.h]{./anexos/interface_mppa.h}
\lstinputlisting[language=C++, label=l6,caption=interface\_mppa.c]{./anexos/interface_mppa.c}
